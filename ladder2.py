...
###### Define new cost function: max entropy function ###
...
class MaxEntropy(Cost):
    @application(outputs=["cost"])
    def apply(self,y_hat):
        cost =  -T.sum(y_hat * T.log(y_hat), axis=y_hat.ndim - 1).mean()
        return cost
######
...
class LadderAE():
    ...
    def apply(self, input_labeled, target_labeled, input_unlabeled):
        ...
        """
        # The max entropy loss for unlabeled data
        print 'using max entropy loss for unlabeled data'
        costs.total += 0.1*MaxEntropy().apply(corr.unlabeled.h[top])
